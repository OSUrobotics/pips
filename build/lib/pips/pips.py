...
import pips.utils.basic
from pips.utils.basic import print_stats
import pips.utils.samp
import pips.utils.misc
...
def balanced_ce_loss(pred, gt, valid=None):
    ...
    pos_loss = pips.utils.basic.reduce_masked_mean(loss, pos*valid)
    neg_loss = pips.utils.basic.reduce_masked_mean(loss, neg*valid)
...
def sequence_loss(flow_preds, flow_gt, vis, valids, gamma=0.8):
    ...
    for i in range(n_predictions):
        ...
        flow_loss += i_weight * pips.utils.basic.reduce_masked_mean(i_loss, valids)
...
class DeltaBlock(nn.Module):
    ...
    def forward(self, fhid, fcorr, flow):
        B, S, D = flow.shape
        assert(D==3)
        flow_sincos = pips.utils.misc.get_3d_embedding(flow, 64, cat_coords=True)
...
class Pips(nn.Module):
    ...
    def forward(self, xys, rgbs, coords_init=None, feat_init=None, iters=3, trajs_g=None, vis_g=None, valids=None, sw=None, return_feat=False, is_train=False):
        ...
        if feat_init is None:
            ...
            ffeat = pips.utils.samp.bilinear_sample2d(fmaps[:,0], coords[:,0,:,0], coords[:,0,:,1]).permute(0, 2, 1) # B, N, C
        else:
            ffeat = feat_init
        ...
        if sw is not None and sw.save_this:
            ...
            for s in range(S):
                if trajs_g is not None:
                    ...
                    kp = pips.utils.improc.draw_circles_at_xy(torch.cat([e_, g_], dim=1), H8, W8, sigma=1).squeeze(2)
                    kp = pips.utils.improc.seq2color(kp, colormap='onediff')
                else:
                    kp = pips.utils.improc.draw_circles_at_xy(coords[0:1,s,0:1], H8, W8, sigma=1).squeeze(2)
                    kp = pips.utils.improc.seq2color(kp, colormap='spring')
                kp = pips.utils.improc.back2color(kp)
                kp_vis.append(kp)
        ...
        if sw is not None and sw.save_this:
            ...
            fcps_ = pips.utils.basic.normalize(fcps_)
            for s in range(S):
